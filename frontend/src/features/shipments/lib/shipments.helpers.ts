...
const SHIPMENT_STATUS_TRANSITIONS: Record<ShipmentStatus, ShipmentStatus[]> = {
  ...
  SHIPPED: ['IN_TRANSIT', 'DISPUTED', 'DELAYED', 'DELIVERED', 'CANCELLED'],
  IN_TRANSIT: ['DISPUTED', 'DELAYED', 'DELIVERED', 'CANCELLED'],
  DISPUTED: ['SHIPPED', 'IN_TRANSIT', 'DELIVERED', 'CANCELLED'],
  DELAYED: ['SHIPPED', 'IN_TRANSIT', 'DISPUTED', 'CANCELLED'],
  DELIVERED: [],
  CANCELLED: [],
};
